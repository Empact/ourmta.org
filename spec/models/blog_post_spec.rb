...
describe BlogPost do
  ...
  describe ".new" do
    it "should create a blog post" do
      BlogPost.new(@json).should be_an_instance_of(BlogPost)    end  end
    
  describe "#title" do
    it { subject.title.should == "FixMTA.org is now OurMTA.org" }  end
  
  describe "#body" do
    it { subject.body.starts_with?("<p>One of many things to come out of last night&#8217;s first planning meeting was the question: who are we, what is our message, and does the name FixMTA suit our goals?</p>").should be_true }  end
  
  describe "#url" do
    it { subject.url.should == "http://blog.ourmta.org/post/201383030/fixmta-org-is-now-ourmta-org" }  end
  
  describe "#posted_on" do
    it { subject.posted_on.should == DateTime.parse('2009-10-01 01:48:13 GMT').to_date }  endend